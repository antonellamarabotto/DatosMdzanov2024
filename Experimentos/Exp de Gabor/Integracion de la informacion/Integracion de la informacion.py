...
IntruccionesComponents = [Instrucciones1, key_resp_3, Instrucciones2, key_resp_4, Instrucciones3, key_resp_5, instrucciones4, key_resp_6, Instrucciones5]
...
while continueRoutine:
    ...
    if Instrucciones5.status == STARTED:
        ...
        if tThisFlipGlobal > Instrucciones5.tStartRefresh + 4-frameTolerance:
            # keep track of stop time/frame for later
...
for thisTraining in Training:
    ...
    GratingPrueba.setOri(Orientacion)
...
for thisTest in Test:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *keyrespTest* updates
        waitOnFlip = False
        if keyrespTest.status == NOT_STARTED and tThisFlip >= 0.2-frameTolerance:
            # keep track of start time/frame for later
            keyrespTest.frameNStart = frameN  # exact frame index
            keyrespTest.tStart = t  # local t and not account for scr refresh
            keyrespTest.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(keyrespTest, 'tStartRefresh')  # time at next scr refresh
            keyrespTest.status = STARTED
            # keyboard checking is just starting
            waitOnFlip = True
            win.callOnFlip(keyrespTest.clock.reset)  # t=0 on next screen flip
            win.callOnFlip(keyrespTest.clearEvents, eventType='keyboard')  # clear events on next screen flip
        if keyrespTest.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > keyrespTest.tStartRefresh + 3.0-frameTolerance:
                # keep track of stop time/frame for later
                keyrespTest.tStop = t  # not accounting for scr refresh
                keyrespTest.frameNStop = frameN  # exact frame index
                win.timeOnFlip(keyrespTest, 'tStopRefresh')  # time at next scr refresh
                keyrespTest.status = FINISHED
        if keyrespTest.status == STARTED and not waitOnFlip:
            theseKeys = keyrespTest.getKeys(keyList=['left','right'], waitRelease=False)
            _keyrespTest_allKeys.extend(theseKeys)
            if len(_keyrespTest_allKeys):
    ...
    Test.addData('keyrespTest.keys',keyrespTest.keys)
    Test.addData('keyrespTest.corr', keyrespTest.corr)
    if keyrespTest.keys != None:  # we had a response
    ...
    routineTimer.add(1.000000)
    # update component parameters for each repeat
    if keyrespTest.corr:#stored on last run routine
      msg="¡Correcto!"
    else:
      msg="Segui intentando"
