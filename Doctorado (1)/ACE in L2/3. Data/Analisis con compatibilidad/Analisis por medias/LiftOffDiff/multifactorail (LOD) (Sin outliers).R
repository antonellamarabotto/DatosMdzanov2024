...
setwd("C:\\Users\\Antonella\\Desktop\\Doctorado\\ACE in L2\\3. Data\\Analisis con compatibilidad\\Analisis por medias\\LiftOffDiff")
...
Tukey1<-TukeyHSD(modeloLiftOffDiffAsentDir)
print(Tukey1)


#Guardo los resultados de Tukey
SenDirGrupoLiftOffDiffCueDir<-as.data.frame(Tukey1$`SenDir:CueDir:GrupoLiftOffDiff`)
write.csv(SenDirGrupoLiftOffDiffCueDir, "TukeyInteraccion.csv", row.names=FALSE, quote=FALSE)

SenDirGrupoLiftOffDiff<-as.data.frame(Tukey1$`SenDir:GrupoLiftOffDiff`)
write.csv(SenDirGrupoLiftOffDiff, "TukeySenDir:GrupoLiftOffDiff.csv", row.names=FALSE, quote=FALSE)

SenDirCueDir<-as.data.frame(Tukey1$`SenDir:CueDir`)
write.csv(SenDirCueDir, "TukeySenDir:CueDir.csv", row.names=FALSE, quote=FALSE)

GrupoLiftOffDiffCueDir<-as.data.frame(Tukey1$`GrupoLiftOffDiff:CueDir`)
write.csv(GrupoLiftOffDiffCueDir, "TukeyGrupoLiftOffDiffCueDir.csv", row.names=FALSE, quote=FALSE)

TukeyGrupoLiftOffDiff<-as.data.frame(Tukey1$GrupoLiftOffDiff)
write.csv(TukeyGrupoLiftOffDiff, "TukeyGrupoLiftOffDiff.csv", row.names=FALSE, quote=FALSE)
...
tukey.cld<-multcompLetters4(modeloLiftOffDiffAsentDir, Tukey1)
print(tukey.cld)
...
tukey.plot.aov<-aov(LiftOffDiff ~ CueDir:GrupoLiftOffDiff:SenDir, data=Dataset)
tukey.plot.test<-Tukey1
par(mar=c(4,6,3,1))
plot(tukey.plot.test, las=1, cex.axis = 0.5)

#Tabla de medias por comparacion
mean.LiftOffDiff.data <- Dataset %>%
  group_by(Grupo.Lift.Off.Diff, CueDirection, SentenceDirection) %>%
  summarise(
    LiftOff = mean(liftOffDiff)
  )

mean.LiftOffDiff.data$group <- c("b","b","b","b","a","a","a","a")
...
#Comparacion posthoc:
lineal<-aov(MoveTime~SentenceDur, data = Dataset)
summary(lineal)
aov(lineal)
#Indepencia de los residuos
